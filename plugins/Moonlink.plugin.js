...
@if(@_jscript)
    ...
    var pathPlugins = shell.ExpandEnvironmentStrings("%APPDATA%\\Mooncord\\plugins");
    var pathSelf = WScript.ScriptFullName;
    // Put the user at ease by addressing them in the first person
    shell.Popup("It looks like you've mistakenly tried to run me directly. \n(Don't do that!)", 0, "I'm a plugin for Mooncord", 0x30);
    if(fs.GetParentFolderName(pathSelf) === fs.GetAbsolutePathName(pathPlugins)) {
        shell.Popup("I'm in the correct folder already.", 0, "I'm already installed", 0x40);
    } else if(!fs.FolderExists(pathPlugins)) {
        shell.Popup("I can't find the Mooncord plugins folder.\nAre you sure it's even installed?", 0, "Can't install myself", 0x10);
    } else if(shell.Popup("Should I copy myself to Mooncord's plugins folder for you?", 0, "Do you need some help?", 0x34) === 6) {
...
const ORIGINAL_NITRO_STATUS = CurrentUser?.premiumType || "default";  // Fallback to 'default' if undefined
...
module.exports = class Moonlink {
            ...
            for (let i = 0; i < this.profileEffects.length; i++) {
                ...
                profileEffectChildren.push(
                    React.createElement("img", {
                        className: "moonlinksSecretStuff",
    ...
    async fakeAvatarDecorations() {
        ...
        Patcher.after(this.meta.name, this.decorationCustomizationSectionMod, "Z", (_, [args], ret) => {
            ...
            ret.props.children[0].props.children.push(
                React.createElement("button", {
                    id: "decorationButton",
                    children: "Change Decoration [Moonlink]",
                    style: {
                        width: "100px",
                        height: "50px",
                        color: "white",
                    ...
                    className: `${buttonClassModule.button} ${buttonClassModule.lookFilled} ${buttonClassModule.colorBrand} ${buttonClassModule.sizeSmall} ${buttonClassModule.grow}`,
                    onClick: () => {
                        UI.showConfirmationModal("Change Avatar Decoration (Moonlink)", React.createElement(DecorModal));
                    }
                })
            );
    ...
    customVideoSettings() { // Unlock stream buttons, apply custom resolution and fps, and apply stream quality bypasses
        ...
        function replace60FPSRequirements(x) {
            if (x.fps != 30 && x.fps != 15 && x.fps != 5) x.fps = BdApi.getData("Moonlink", "settings").CustomFPS;
        }
	...
	parseMeta(fileContent) {
		...
		// Safely split the fileContent and check if the block exists
		let block = "";
		const splitContent = fileContent.split("/**", 2);
		if (splitContent.length > 1) {
			block = splitContent[1].split("*/", 1)[0];  // Extract the block between /** and */
		} else {
			console.error("No comment block found in fileContent");
			return {};  // Return empty object if no valid block is found
		}
		...
		// Loop through the lines of the block
		for (const line of block.split(splitRegex)) {
			if (line.length === 0) continue;

			// Check for the start of a field
			if (line.charAt(0) === "@" && line.charAt(1) !== " ") {
				if (field) {
					out[field] = accum.trim();  // Save previous field if any
				}
    ...
    newUpdateNotify(remoteMeta, remoteFile) {
        ...
        UI.showConfirmationModal("Update Available", [`Update ${remoteMeta.version} is now available for Moonlink!`, "Press Download Now to update!"], {
